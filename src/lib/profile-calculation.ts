...
// Define a type that covers both PrismaClient and the TransactionClient
type PrismaTx = Omit<PrismaClient, '$connect' | '$disconnect' | '$on' | '$transaction' | '$use' | '$extends'>

export async function calculateProfileWeightInternal(
    tx: PrismaTx,
    type: string,
    params: { gradeId?: string, w?: number, h?: number, t?: number, d?: number, s?: number, dimensions?: string }
) {
    // 1. Determine Density
    let density = 7.85 // kg/dm3 default
    if (params.gradeId) {
        const grade = await tx.materialGrade.findUnique({
    ...
    if (params.dimensions) {
        ...
        const standard = await tx.standardProfile.findUnique({
    ...
    const shape = await tx.profileShape.findUnique({ where: { id: type } })
